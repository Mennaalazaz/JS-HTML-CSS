...
form.addEventListener("submit", function (e) {
  // A click on a form submit button – initiates its submission to the server so we wanna prevent it
  e.preventDefault();

  let number = parseInt(amount.value); // convert the returning string to int

  const randomIdx = Math.floor(Math.random()*text.length);

  // if no value is entered or the value is in wrong range .. return any single random text
  if (isNaN(number) || number<=0 || number>text.length){
    loremText.innerHTML =  `<p class='result'>${text[randomIdx]}</p>`;
  }
  else{
    let tempText = text.slice(0,number); //get array of strings from the text array from index:0 till index:number
